...
private:
    ...
    vector<pair<string, int>> global_names;
    ...
    void declareGlobal(const string &name, bool is_array = false, int size = 1);
    void declareLocal(const string &name, bool is_array = false, int size = 1);
    void declareParameter(const string &name);
    string operandFor(SymbolInfo *symbol);
    string resolveVariableAddress(CSubsetParser::VariableContext *ctx);
    int nextLabelIndex();
...
public:
    ...
    any visitDeclarationSingleScalarWithInit(CSubsetParser::DeclarationSingleScalarWithInitContext *ctx) override;
    any visitDeclarationAppendScalarWithInit(CSubsetParser::DeclarationAppendScalarWithInitContext *ctx) override;
    ...
    any visitSinglePrint(CSubsetParser::SinglePrintContext *ctx) override;
    any visitPrintList(CSubsetParser::PrintListContext *ctx) override;
