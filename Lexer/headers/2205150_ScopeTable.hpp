...
class ScopeTable{
    ...
    public:
        ScopeTable(string scope_table_uid, ui bucket_numbers, ScopeTable *parent_scope, int hash_function = 1){
            ...
            if(hash_table[row_index] == NULL){
                ...
                while(current != NULL){
                    if(current->getSymbolName() == symbol_name){
                        cout << "< " << symbol_name << " : " << symbol_type << " > already exists in ScopeTable# " << this->scope_table_uid << " at position " << row_index << ", " << col_index << endl << endl;
        ...
        SymbolInfo* lookUp(const string &str){
            ...
            while(current != NULL){
                if(current->getSymbolName() == str){
                    cout << "\t" << "'" << str << "'" << " found in ScopeTable# " << this->scope_table_uid << " at position " << row_index << ", " << col_index << endl;
                    return current;
                }
        ...
        bool deleteSymbol(const string &str){
            ...
            if(current != NULL && current->getSymbolName() == str){
                ...
                cout << "\t" << "Deleted " << "'" << str << "'" << " from ScopeTable# " << this->scope_table_uid << " at position " << row_index << ", " << col_index << endl;
        ...
        void printScopeTable(int tab = 0){
            ...
            // Print each bucket (only if non-empty)
            for (ui i = 0; i < bucket_numbers; i++)
            {
